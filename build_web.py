...
<body class="bg-white text-[#202124] min-h-full flex flex-col antialiased">
  ...
  <!-- Utility Header (Intuitive style) -->
  ...
  <!-- Primary Navigation Bar (Intuitive Clean White Style) -->
        ...
        <div class="bg-white rounded-xl border border-[#e5e7eb] p-5 shadow-xs space-y-4">
          ...
          <div class="space-y-1.5">
            ...
            <div class="grid grid-cols-3 gap-1.5">
              <button data-noise="white" class="noise-type-btn px-2.5 py-2 text-xs font-semibold rounded-lg bg-[#202124] text-white transition shadow-xs">White</button>
              <button data-noise="pink" class="noise-type-btn px-2.5 py-2 text-xs font-semibold rounded-lg bg-[#f2f3f5] hover:bg-[#e5e7eb] text-[#202124] border border-[#e5e7eb] transition">Pink (1/f)</button>
              <button data-noise="fan" class="noise-type-btn px-2.5 py-2 text-xs font-semibold rounded-lg bg-[#f2f3f5] hover:bg-[#e5e7eb] text-[#202124] border border-[#e5e7eb] transition">Fan Hum</button>
            </div>
          </div>
        ...
        <div class="bg-white rounded-xl border border-[#e5e7eb] p-6 shadow-xs space-y-4">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#f2f3f5] pb-3">
            ...
            <div class="flex items-center gap-1 bg-[#f2f3f5] p-1 rounded-lg border border-[#e5e7eb]">
              <button id="tabNoisySpec" class="px-3 py-1 text-xs font-semibold rounded-md text-[#55575c] hover:text-[#202124] transition">
                Noisy
              </button>
              <button id="tabEnhancedSpec" class="px-3 py-1 text-xs font-semibold rounded-md bg-white text-[#1c23ba] shadow-2xs transition">
                Enhanced
              </button>
              <button id="tabCleanSpec" class="px-3 py-1 text-xs font-semibold rounded-md text-[#55575c] hover:text-[#202124] transition">
                Clean
              </button>
            </div>
          </div>
  ...
  <script>
    ...
    function processDSP() {
      ...
      const activeNoiseBtn = document.querySelector(".noise-type-btn.bg-\\[\\#202124\\]");
    ...
    function setupUI() {
      ...
      noiseBtns.forEach(btn => {
        btn.addEventListener("click", () => {
          noiseBtns.forEach(b => {
            b.className = "noise-type-btn px-2.5 py-2 text-xs font-semibold rounded-lg bg-[#f2f3f5] hover:bg-[#e5e7eb] text-[#202124] border border-[#e5e7eb] transition";
          });
          btn.className = "noise-type-btn px-2.5 py-2 text-xs font-semibold rounded-lg bg-[#202124] text-white transition shadow-xs";
          const type = btn.getAttribute("data-noise");
          document.getElementById("noiseTypeDesc").innerText = 
            type === "white" ? "Gaussian Broadband" : (type === "pink" ? "1/f Pink Noise" : "Narrowband Fan Hum");
          processDSP();
        });
      });
      ...
      function setSpecTab(active) {
        ...
        tabNoisy.className = active === "noisy" ? "px-3 py-1 text-xs font-semibold rounded-md bg-white text-[#1c23ba] shadow-2xs transition" : "px-3 py-1 text-xs font-semibold rounded-md text-[#55575c] hover:text-[#202124] transition";
        tabEnhanced.className = active === "enhanced" ? "px-3 py-1 text-xs font-semibold rounded-md bg-white text-[#1c23ba] shadow-2xs transition" : "px-3 py-1 text-xs font-semibold rounded-md text-[#55575c] hover:text-[#202124] transition";
        tabClean.className = active === "clean" ? "px-3 py-1 text-xs font-semibold rounded-md bg-white text-[#1c23ba] shadow-2xs transition" : "px-3 py-1 text-xs font-semibold rounded-md text-[#55575c] hover:text-[#202124] transition";
